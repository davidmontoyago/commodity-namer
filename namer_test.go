...
func TestNewResourceName_WithReplace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "replace forward slashes in service name",
			baseName:     "app",
			serviceName:  "api/v1/auth",
			resourceType: "service",
			maxLength:    50,
			expected:     "app-api-v1-auth-service",
			description:  "should replace forward slashes with dashes in service name",
		},
		{
			name:         "replace forward slashes in resource type",
			baseName:     "app",
			serviceName:  "service",
			resourceType: "api/gateway/instance",
			maxLength:    50,
			expected:     "app-service-api-gateway-instance",
			description:  "should replace forward slashes with dashes in resource type",
		},
		{
			name:         "mixed forward slashes, periods, and underscores",
			baseName:     "app",
			serviceName:  "api/v1.auth_service",
			resourceType: "gateway/ssl.instance_type",
			maxLength:    50,
			expected:     "app-api-v1-auth-service-gateway-ssl-instance-type",
			description:  "should replace all special characters (/, ., _) with dashes",
		},
		{
			name:         "forward slashes with uppercase conversion",
			baseName:     "app",
			serviceName:  "API/V1/AUTH.Service",
			resourceType: "Gateway/SSL_Instance",
			maxLength:    50,
			expected:     "app-api-v1-auth-service-gateway-ssl-instance",
			description:  "should replace slashes and convert to lowercase",
		},
		{
			name:         "path-like service names",
			baseName:     "microservice",
			serviceName:  "users/profiles/auth",
			resourceType: "api/endpoint",
			maxLength:    50,
			expected:     "microservice-users-profiles-auth-api-endpoint",
			description:  "should handle path-like naming patterns",
		},
		{
			name:         "multiple consecutive forward slashes",
			baseName:     "app",
			serviceName:  "api//v1//auth",
			resourceType: "gateway//instance",
			maxLength:    50,
			expected:     "app-api--v1--auth-gateway--instance",
			description:  "should replace consecutive forward slashes with consecutive dashes",
		},
		{
			name:         "forward slashes with truncation",
			baseName:     "app",
			serviceName:  "very/long/api/service/name",
			resourceType: "gateway/ssl/instance/type",
			maxLength:    25,
			expected:     "a-very-long-a-gateway-ssl",
			description:  "should replace slashes before truncation",
		},
		{
			name:         "kubernetes-style names with slashes",
			baseName:     "cluster",
			serviceName:  "apps/v1/deployment",
			resourceType: "networking/v1/ingress",
			maxLength:    50,
			expected:     "cluster-apps-v1-deployment-networking-v1-ingress",
			description:  "should handle Kubernetes API-style naming",
		},
		{
			name:         "docker image style names",
			baseName:     "registry",
			serviceName:  "myorg/myapp",
